...
lmd = 20.  # weight for swap loss
ratio = opts.ratio / 100.0
wts = 'wts/unsupervised_ratio%d'%(opts.ratio)
...
tset = Dataset(TLIST, BSZ, IMSZ, CSZ, niter)
...
rec_loss, swap_loss = get_loss(left_signal, right_signal, left_recon, right_recon, shifts, mat, CSZ)
...
while niter < MAXITER and not ut.stop:
    ...
    if niter % 100 == 0:
        ...
        outs = sess.run(
            [loss, tStep, tset.fetchOp],
            feed_dict={**tset.fdict(), lr: get_lr(niter), is_training: True}
        )
        ut.vprint(niter, ['loss.t'], outs[:1])
